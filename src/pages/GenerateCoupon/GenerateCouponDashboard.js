...
const GenerateCouponDashboard = () => {
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState("");
  const [customers, setCustomers] = useState([]); // Removed type annotations
  ...
  const handleCopy = (generateNo, e, customer) => {
          ...
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              {filteredCustomers.map((customer) =>
                customer.CustomerDetails.map((customerDetail, index) => (
                  <tr key={`${customer._id}-${index}`}>
                    ...
                    <td className="border border-[#6D4222] pl-1 text-center">
